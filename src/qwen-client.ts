...
import type { QwenMessage, QwenModelId, QwenStreamEvent, QwenTool, QwenToolChoice } from './types'
...
class QwenClient {
  ...
  async *streamChatCompletion(params: {
    ...
    for await (const chunk of stream) {
      ...
      for (const call of delta?.tool_calls ?? []) {
        ...
        const current = toolCalls.get(index) ?? { id: call.id ?? `tool_call_${index}`, arguments: '' }
  ...
  async countTokens(messages: QwenMessage[]): Promise<number> {
    ...
    for (const message of messages) {
      ...
      totalChars += match(content)
        .when((value): value is string => typeof value === 'string', (value) => value.length)
      ...
      totalChars += ((message as { tool_calls?: Array<{ function?: { name?: string; arguments?: string } }> })
        .tool_calls ?? [])
        .reduce(
          (sum, call) => sum + (call.function?.name?.length ?? 0) + (call.function?.arguments?.length ?? 0),
          0,
        )
    }
  ...
  private parseToolInput(raw: string): object {
    return match(destr(raw))
      .with(P.nullish, () => ({}))
      .when((value) => typeof value === 'object', (value) => (value as object) ?? {})
