...
const isDeviceAuthorizationData = (
  value: unknown,
): value is DeviceAuthorizationData =>
  isRecord(value) &&
...
class QwenAuthHandler {
  ...
  async loadCredentials(): Promise<QwenCredentials | null> {
    ...
    await toResult(
      this.secretStorage?.store(SECRET_STORAGE_KEY, JSON.stringify(fromFile)),
    )
  ...
  getBaseUrl(): string {
    ...
    const normalized = /^https?:\/\//.test(resourceUrl)
      ? resourceUrl
      : `https://${resourceUrl}`;
  ...
  async startDeviceFlow(params: {
    ...
    while (Date.now() < deadline) {
      ...
      const result = match<DeviceTokenResponse, QwenCredentials | 'pending'>(
        tokenResponse,
      )
        .with({status: 'pending'}, pending => {
          pollIntervalMs = pending.slowDown
            ? Math.min(Math.floor(pollIntervalMs * 1.5), 10000)
            : pollIntervalMs;
  ...
  private async refreshAccessToken(): Promise<void> {
    ...
    if (!isTokenResponse(data)) {
      throw new Error(
        `Token refresh failed: invalid response ${toStringPayload(response.data)}`,
      );
    }
  ...
  private async requestDeviceAuthorization(
    ...
    const response = await this.postForm<
      DeviceAuthorizationData | OAuthErrorResponse
    ...
    return match(response.data)
      ...
      .otherwise(() => {
        throw new Error(
          `Device authorization failed: invalid response ${toStringPayload(response.data)}`,
        );
      });
  }
  ...
  private async pollDeviceToken(
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      throw new Error(
        `Device token poll failed: invalid response ${toStringPayload(response.data)}`,
      );
    }
    ...
    return match<[number, string], DeviceTokenResponse>([
      response.status,
      oauthError.error,
    ])
  ...
  private async writeCredentialsToDisk(
